...
function win() {
	...
	setTimeout(() => {scene.transition(0, (Glass.get("Overworld") as Scene), {playerMove: true})}, 500)
}
...
function throwDice(type: number) {
	...
	popupSprite.script?.takeData(popupSprite, {roll: true, type})
}
function threwDice() {
	const amount = rolled * dice[currType / 2 - 2]
	box.addOptions(`you rolled for ${amount} hp` + (dice[currType / 2 - 2] > 1 ? `, since you have ${dice[currType / 2 - 2]} dice!` : "!"), ["damage enemy", "heal myself"], (i) => {
		//  damage to ${enemies[0].type}
		if (i == 0) { enemies[0].health -= amount, box.addOptions(`you did ${amount} damage to ${enemies[0].type}`, ["ok"], () => enemyTurn()) }
		if (i == 1) { player.health += amount,     box.addOptions(`you healed yourself by ${amount} hp`, ["ok"], () => enemyTurn()) }
	})
}

function itemBox() {
	box.addOptions("you have no items!", ["ok"], () => mainBox())
}
...
function enemyTurn(on = 0) {
	...
	box.addOptions(`${enemies[0].type} did ${enemyRoll} damage!`, ["ok"], () => mainBox())
}
...
export function setup(self: Scene) {
	...
	self.has(
		new TileMap([
			...
			], "Assets/battleMap.png", 32, 16).name("BattleTileMap")
			.has(playerSprite),
		new OptionBox(1, 1, 1, 0.3, true).name("BattleBox"),
		new Sparkle(200).name("BattlePopup")
			.has(
				new Sprite("Assets/dice.png").name("PopupSprite").onLoad(sp => {
					sp.rect.width = 10
					sp.size.x = 96
					sp.size.y = 96
					sp.pos.set(42, 42)
					sp.center()
				}).setScript("battlePopup")
			).hide(),
		new Sparkle(200).name("WinPopup")
			.has(
				new Sprite("Assets/win.png").name("PopupSprite").onLoad(sp => {
					sp.size.set(96, 64)
					sp.center()
				}).setScript("battlePopup")
			).hide()
	)
	popupSprite = Glass.get("PopupSprite") as Sprite
	tilemap = Glass.get("BattleTileMap") as TileMap
	box = Glass.get("BattleBox") as OptionBox
	playerSprite.tpTo(16, 13)
}

export function frame() {
	if (rolled == 0 && !(Glass.get("BattlePopup") as Sparkle).visible) {
		/// @ts-ignore
		rolled = (popupSprite.script as Module).rollNumber
		threwDice()
	}
