...
class Console {
	...
	constructor(desiredHeight, backgroundColor) {
		...
		// Different functions.
		this.initFn    = () => { void 0 }
		this.preFrameFn = () => { void 0 }
		this.frameFn    = () => { void 0 }
		this.pFrameFn   = () => { void 0 }
		this.frameCount = 0
	}
	 ...
	 * @param funct Function to be ran when the event is triggered
	 */
	nEvent(name, funct) {
		if (name in this.events) CTool.warning("Event '" + name + "' already exists. Its function was replaced.")
		this.events[name] = [funct || (() => void 0 ), false]
	}

	/**
	 * Runs an event when a key is pressed.
	 * @param keyName Name of the key to to check for
	 * @param eventName Name of the event to run
	 */
	...
	eventOngoing(eventName) {
		if (!(eventName in this.events)) {
			...
			this.events[eventName] = [() => void 0, false]
			return false
		}
	 ...
	 * @param height Height of touch area array
	 * @param width Width of touch area array
	 * @param events Array of events to run when the screen is touched
	 */
...
class TileSet {
	 ...
	 * @param src Source for the tileset
	 * @param tw Tile width
...
class TileMap {
	 ...
	 * @param px Pixel data in RGBA form.
	 * @param w Width of pixel data
	 * @param types Types of tiles
	 */
	 ...
	 * @param [fallBack] Fallback when there's no tile found
	 ...
	 * @param cn Number of collision box to get
...
class CTool {
	 ...
	 * @param p1 Parameter 1 (light value, light value, red, red)
	 * @param p2 Parameter 2 (alpha, green, green)
	 * @param p3 Parameter 3 (blue, blue)
	 * @param p4 Parameter 4 (alpha)
	 * @returns 
	 */
	 ...
	 * @param n Number to turn to binary
	 * @param [l] Minimum length of string to return. Defaults to 0
	 * @returns {String}
	 */
	static bin(n, l) {
		let r = n.toString(2)
	 ...
	 * @param [d] Decimal places. Defaults to zero.
	 * @returns {number}
	 */
	static round(n, d) {
		d = 10 ** (d || 0)
	 ...
	 * @param v Percent to interpolate between `a` and `b`
	 * @returns {number}
	 */
	static lerp(a, b, v) {
		return a * (1 - v) + b * v
	 ...
	 * @param ir1 Start of initial range
	 * @param ir2 End of initial range
	 * @param or1 Start of target range
	 * @param or2 End of target range
	 * @returns {number}
	 */
	 ...
	 * @param src Source of the image to load
	 * @param callback Callback called when the image pixels are gotten
	 */
	 ...
	 * @param str Data string that contains the image data.
...
class SoundPlayer {
	...
	pause(path) {
		this.loaded[path].ontimeupdate = () => void 0
	...
	static new(element, type) {
		...
		for (let evt in types[type]) {
			if (evt == "_touch") {
				...
				element.parentCon.nEvent(evt, () => void 0)
			}
		}
	}
